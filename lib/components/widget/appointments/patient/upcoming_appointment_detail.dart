...
class UpcomingAppointmentDetail extends StatelessWidget {
  ...
  final Function(PatientAppointmentModel) hoursUntilAppointment;
  final Function(int price) formatPrice;
  final Function(BuildContext, PatientAppointmentModel, int, String, bool, bool) showBlurDialog;
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Text(
            '${hoursUntilAppointment(appointment).toStringAsFixed(0)}h',
            style: const TextStyle(
              fontSize: 13,
      ...
      children: [
        ...
        Column(
          ...
          children: [
            ...
            if (appointment.meetType.contains('Khám tại nhà')) ...[
              const SizedBox(height: 5),
              Text(appointment.address, style: const TextStyle(color: AppColors.primaryText)),
            ] else if (appointment.meetType.contains('Khám tại phòng khám')) ...[
              const SizedBox(height: 5),
              Text(appointment.officeAddress, style: const TextStyle(color: AppColors.primaryText)),
            ] else ...[
              const SizedBox(height: 5),
              Text(appointment.link, style: const TextStyle(color: AppColors.primaryText)),
            ],
          ],
        ),
      ],
  ...
  Widget _buildActionButtons(BuildContext context) {
    return CustomButtonPlus(
      onTap: () => showBlurDialog(context, appointment, index, "cancelled", true, true),
      height: 50,
      color: AppColors.primaryText,
      btnText: 'cancel'.tr,
      fontFamily: AppFontStyleTextStrings.bold,
      ...
      isDisabled: hoursUntilAppointment(appointment) < 1,
    );
  }
}
